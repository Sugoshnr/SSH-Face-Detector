...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, module, img):
	...
	resized_width = resized_height = 224
	# rpn ground truth
		...
		for anchor_ratio_idx in range(n_anchratios):
			...
			for ix in range(output_width):					
				...
				for jy in range(output_height):
					...
					# if C.diagnose:
					# 	for bbox_num in range(num_bboxes):
					# 		color = {'pos': (255, 0, 0), 'neutral': (255, 255, 255), 'neg': (0, 0, 255)}
					# 		if bbox_type == 'pos':# or bbox_type == 'neutral':
					# 			print('bbox_type = ', bbox_type)
					# 			x1_ab, x2_ab, y1_ab, y2_ab = map(int, best_x_for_bbox[bbox_num])
					# 			cv2.rectangle(img, (x1_ab, y1_ab), (x2_ab, y2_ab), color[bbox_type], thickness=1)
	...
	if C.diagnose:
		...
		f = module+img_data['filepath'].split(os.sep)[-1]
		print((os.path.join(os.sep.join(p), 'diagnose', f)))
		height, width = img.shape[:2]
		cv2.imwrite(os.path.join(os.sep.join(p), 'diagnose', f), img)
		img = cv2.resize(img,(4*width, 4*height), interpolation = cv2.INTER_CUBIC)
		cv2.imwrite(os.path.join(os.sep.join(p), 'diagnose', 'resize'+f), img)
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				# x_img = final_image.astype('uint8')
				try:
					y_rpn_cls, y_rpn_regr = calc_rpn(C, img_data_aug, width, height, resized_width, resized_height, 'M1', np.copy(x_img))
					y_rpn_cls2, y_rpn_regr2 = calc_rpn(C, img_data_aug, width, height, resized_width, resized_height, 'M2', np.copy(x_img))
					y_rpn_cls3, y_rpn_regr3 = calc_rpn(C, img_data_aug, width, height, resized_width, resized_height, 'M3', np.copy(x_img))
					# print('Success')
